...
old = """  const [schoolLevel, setSchoolLevel] = useState('');
  const [schoolYear, setSchoolYear] = useState('');
"""
new = """  const [schoolLevel, setSchoolLevel] = useState('');
  const [schoolYear, setSchoolYear] = useState('');
  const [higherEducationCourse, setHigherEducationCourse] = useState('');
"""
if old not in text:
    raise SystemExit('School state block not found')
text = text.replace(old, new, 1)

old = """  const [municipalities, setMunicipalities] = useState<string[]>([]);
  const [municipalitiesError, setMunicipalitiesError] = useState(false);
"""
new = """  const [municipalities, setMunicipalities] = useState<string[]>([]);
  const [municipalitiesError, setMunicipalitiesError] = useState(false);
...
"""
if old not in text:
    raise SystemExit('Municipality state block not found')
text = text.replace(old, new, 1)

old = """  const selectedGameVerified = !selectedGameNeedsExactVersion && selectedGameClassind?.status === 'verified' && selectedGameMinimumAge !== null && selectedGameFresh;
...
"""
if old not in text:
    raise SystemExit('Computed game block not found')
text = text.replace(old, new, 1)
...
marker = """  function handleLogin(event: FormEvent<HTMLFormElement>) {
"""
insert = """  useEffect(() => {
...
old = """    const schoolYearRequired = schoolLevel === 'Ensino fundamental' || schoolLevel === 'Ensino médio';
...
"""
new = """    const schoolYearRequired = schoolLevel === 'Ensino fundamental' || schoolLevel === 'Ensino médio';
...
"""
if old not in text:
    raise SystemExit('School validation block not found')
text = text.replace(old, new, 1)

school_title = '<h4>Vínculo escolar</h4>'
title_pos = text.find(school_title)
if title_pos < 0:
    raise SystemExit('Vínculo escolar title not found')
start = text.rfind('                      <section className="glass-card form-section">', 0, title_pos)
end_marker = "\n\n                      {minorStatus === true && ("
end = text.find(end_marker, title_pos)
if start < 0 or end < 0:
    raise SystemExit('School section boundaries not found')

new_section = '''                      <section className="glass-card form-section">
