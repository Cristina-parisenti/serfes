...
function calculateIsMinor(dateValue: string): boolean | null {
...
export function AppV2() {
  ...
  const [termGeneratedDate, setTermGeneratedDate] = useState('');
  ...
  const isMinor = calculateIsMinor(athleteBirthDate);
  ...
  function handleAthleteSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const responsibleFieldsValid = isMinor !== true || (
  ...
  function prepareAuthorization() {
    if (isMinor !== true) return;
    setTermGeneratedDate(formatLongDate(new Date()));
    setTermVisible(true);
    setSignatureStatus('Pronto para assinatura');
    setTimeout(() => document.querySelector('.term-preview')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 50);
  }

  const automaticLocation = athleteMunicipality ? `${athleteMunicipality}/PR` : '[MUNICÍPIO]/PR';
  const ageStatus = isMinor === null ? 'Aguardando data de nascimento' : isMinor ? 'Menor de 18 anos' : '18 anos ou mais';
            ...
            {dashboardSection === 'athleteHome' && isAthlete && (
              <>
                ...
                <section className="athlete-summary-grid">
                  ...
                  <article className="mini-status red"><strong>{isMinor ? signatureStatus : '—'}</strong><span>Autorização responsável</span></article>
                </section>
                <section className="panel-grid">
                  ...
                  <article className="glass-card panel-card">
                    ...
                    <div className="list-card"><strong>2</strong><span>Se for menor de idade, o SERFES habilitará automaticamente a autorização.</span></div>
                ...
                <form className="athlete-form" onSubmit={handleAthleteSubmit}>
                  ...
                  <section className="glass-card form-section">
                    ...
                    <div className="form-grid">
                      ...
                      <label>Data de nascimento
                        <input required type="date" max={todayInputMax()} value={athleteBirthDate} onChange={(e) => setAthleteBirthDate(e.target.value)} />
                        <small className="field-help">A idade é calculada automaticamente pelo SERFES.</small>
                      </label>
                      <label>Situação etária<input readOnly value={ageStatus} className={isMinor ? 'age-minor' : ''} /></label>
                      <label>CPF
                  ...
                  {isMinor === true && (
                    <section className="glass-card form-section authorization-section">
                      <div className="form-section-title"><div className="icon-box red-box"><ShieldCheck size={20} /></div><div><h4>Responsável legal e autorização</h4><p>Esta etapa foi habilitada automaticamente porque a data de nascimento indica idade inferior a 18 anos.</p></div></div>
                      <div className="auto-minor-banner"><CheckCircle2 size={18} /><span>Menor de 18 anos identificado automaticamente. O termo será preenchido com os dados abaixo e com os dados já informados no cadastro.</span></div>
                      <div className="form-grid">
                        <label>Situação do termo<input readOnly value={signatureStatus} /></label>
                        <label>Local do termo<input readOnly value={automaticLocation} /></label>
                  ...
                  {isMinor === false && athleteBirthDate && (
                    <section className="glass-card form-section adult-confirmation">
                      <div className="form-section-title"><div className="icon-box green-box"><CheckCircle2 size={20} /></div><div><h4>Autorização do responsável</h4><p>Não se aplica.</p></div></div>
                      <div className="auto-adult-banner">A data de nascimento indica que o atleta possui 18 anos ou mais. O SERFES não exige termo de responsável legal para este cadastro.</div>
                    </section>
                  )}

                  {isMinor === true && termVisible && (
                    <section className="term-preview" aria-label="Pré-visualização do termo de autorização">
                      ...
                      <article className="term-paper">
                        ...
                        <p>Eu, <strong>{responsibleName || '[NOME DO RESPONSÁVEL LEGAL]'}</strong>, portador(a) do RG nº <strong>{responsibleRg || '[RG DO RESPONSÁVEL]'}</strong> e inscrito(a) no CPF nº <strong>{responsibleCpf || '[CPF DO RESPONSÁVEL]'}</strong>, na qualidade de responsável legal pelo(a) estudante <strong>{athleteName || '[NOME DO(A) ESTUDANTE]'}</strong>, regularmente matriculado(a) na instituição de ensino <strong>{athleteInstitution || '[INSTITUIÇÃO DE ENSINO]'}</strong>, autorizo sua participação voluntária nos Jogos Eletrônicos Escolares, a serem realizados no período de <strong>{competitionPeriod || '[PERÍODO DA COMPETIÇÃO]'}</strong>.</p>
                        <p>Declaro estar ciente de que a iniciativa possui caráter educacional, formativo e recreativo, sendo organizada pela Secretaria de Esportes do Estado do Paraná, em parceria com <strong>{partnerInstitution || '[INSTITUIÇÃO PARCEIRA]'}</strong>, com o objetivo de promover o desenvolvimento de competências digitais, sociais e esportivas.</p>
                        ...
                        <div className="term-fields">
                          <p><strong>{automaticLocation}, {termGeneratedDate || formatLongDate(new Date())}.</strong></p>
                          <p><strong>Assinatura eletrônica do responsável legal</strong></p>
                        </div>
                      </article>

                      <div className="signature-flow signature-flow-inline no-print">
                        <div className="signature-step complete">
                          <span>1</span>
                          <div><strong>Termo preenchido</strong><small>Dados do cadastro e a data foram inseridos automaticamente pelo SERFES.</small></div>
                        </div>
                        <div className="signature-step current gov-signature-step">
                          <span>2</span>
                          <div>
                            <strong>Assinatura com GOV.BR</strong>
                            <small>O responsável assinará o documento eletronicamente quando a integração institucional estiver habilitada.</small>
                  ...
                  <section className="glass-card form-section">
                    ...
                    <div className="document-grid">
                      ...
                      <div className="document-item"><FileSignature size={18} /><div><strong>Autorização do responsável</strong><span>{isMinor === true ? signatureStatus : 'Não se aplica'}</span></div></div>
                    </div>
